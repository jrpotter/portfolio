...
readRaw :: forall m. MonadAff m
        => PostCardR -> ExceptT String m PostCard.PostCard
readRaw (PostCardR { title, description, createdAt, updatedAt, slug }) = do
  ...
  where
    convert :: forall m'. MonadAff m' => String -> ExceptT String m' DateTime
    convert dt = do
...
type State = Array PostCard.PostCard
...
render state = HH.div [ HP.id_ "post-list" ] $
  map (\p -> HH.slot postProxy 0 PostCard.component p absurd) state
